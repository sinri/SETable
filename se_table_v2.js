/**
 * Sinri Edogawa Table Version II
 * -------------------------------
...
var se_table=function(target_div_id, sheets_data,config){
	...
	if(config){
		...
		//for v2
		if(config.table){
			if(config.table.width){
				this.config.table_width=config.table.width;
			}
		}
	}
	...
	if(config.tables){
		for(var table_name in config.tables){
			...
			//for v2
			this.config[table_name].table_width=config.tables[table_name].table_width;
		}
	}
	...
	this.createTable=function(table_obj,sheet_index,table_index){
		...
		if(that.config['of_'+sheet_index+"_"+table_index]){
			...
			table_config.scroll_size=that.config.scroll_size;
		}

		// console.log(table_obj);
		// var left_cell_cols=table_obj.fixed_left_cols?:1;

		html+="<div id='"+TABLE_DIV_ID+"' class='SE_TABLE_ATOMTABLE_DIV'>";//beginning of SE_TABLE_ATOMTABLE_DIV

		html+="<table style='width:"+table_config.table_width+"px' class='table table-bordered'>";
		html+="<thead>";

		for(var r_index in table_obj.header){
			html+="<tr>";
			for(var c_index in table_obj.header[r_index]){
				var cell_obj=table_obj.header[r_index][c_index];
				if(typeof cell_obj == 'object'){
					html+="<th ";
					if(cell_obj.rowspan){
						html+="rowspan='"+cell_obj.rowspan+"' ";
					}
					if(cell_obj.colspan){
						html+="colspan='"+cell_obj.colspan+"' ";
					}
					// if(!cell_obj.style_id){
					// 	cell_obj.style_id='header_cell';
					// 	if(c_index==0)cell_obj.style_id='corner_cell';
					// }
					if(cell_obj.style_id){
						html+=" class='"+cell_obj.style_id+"' ";
					}
					html+=">";
					html+=cell_obj.value;
				}else{
					html+="<th class='header_cell'>";
					html+=cell_obj;
				}
				html+="</th>";
			}
		...
		for(var r_index in table_obj.body){
			html+="<tr>";
			// var col_count=0;
			for(var c_index in table_obj.body[r_index]){
				var cell_obj=table_obj.body[r_index][c_index];
				if(typeof cell_obj == 'object'){
					// if(col_count==0){
					// 	cell_obj.style_id='left_cell';
					// }
					html+="<td ";
					if(cell_obj.rowspan){
						html+="rowspan='"+cell_obj.rowspan+"' ";
		...
		for(var r_index in table_obj.tail){
			html+="<tr>";
			var col_count=0;
			for(var c_index in table_obj.tail[r_index]){
				var cell_obj=table_obj.tail[r_index][c_index];
				if(typeof cell_obj == 'object'){
					html+="<td ";
					if(cell_obj.rowspan){
						html+="rowspan='"+cell_obj.rowspan+"' ";
		...
		html+="</div>";//end of SE_TABLE_ATOMTABLE_DIV

		that.format_task[TABLE_DIV_ID]=function(){
			$("#"+TABLE_DIV_ID).css('width',table_config.box_width);
			$("#"+TABLE_DIV_ID).css('height',table_config.box_height);


			$('#'+TABLE_DIV_ID).scroll(function(event){
				// console.log('CT.scrollLeft='+event.currentTarget.scrollLeft+' CT.scrollTop='+event.currentTarget.scrollTop);
				$('#'+TABLE_DIV_ID+' .corner_cell').css('top',Math.max(0,(event.currentTarget.scrollTop-1))+'px');
				$('#'+TABLE_DIV_ID+' .corner_cell').css('left',Math.max(0,(event.currentTarget.scrollLeft-1))+'px');
				$('#'+TABLE_DIV_ID+' .header_cell').css('top',Math.max(0,(event.currentTarget.scrollTop-1))+'px');
				$('#'+TABLE_DIV_ID+' .left_cell').css('left',Math.max(0,(event.currentTarget.scrollLeft-1))+'px');
			})
		}

		console.log('GUHEHE> '+html)

		return html;
	}	

	this.createTableOld=function(table_obj,sheet_index,table_index){
		var TABLE_DIV_ID="SE_TABLE_ATOMTABLE_DIV_"+sheet_index+"_"+table_index;
		...
		if(table_obj.fixed_left){
			...
			if(table_obj.corner_span instanceof Array){
				...
				for(var rk in table_obj.corner_span){
					...
					for(var ck in table_obj.corner_span[rk]){
						...
						html+="<th rowspan='"+rowspan+"' colspan='"+colspan+"' style='width:"+(table_config.cell_width*colspan)+"px;height:"+(table_config.cell_height*rowspan-2+2)+"px'>";
						...
						html+="<input class='"+sort_input_class+"' value='"+input_value+"'>";

						if(rk!=table_obj.corner_span.length-1 || table_config.no_sort){
							html+="<div class='SE_TABLE_HEADER_CELL_SORT_DIV SE_TABLE_INVISIBLE'>";
						}else{
							//SORT?
							html+="<div class='SE_TABLE_HEADER_CELL_SORT_DIV'>";
						}
						...
						html+="<div class='SE_TABLE_HEADER_CELL_SORT_DIV_UP' ";
						html+=" onclick='SETable.sortForColumn(\"SE_TABLE_ATOMTABLE_DIV_"+sheet_index+"_"+table_index+"\","+(-1-ck)+",\"asc\")'></div>";//↑
						html+="<div class='SE_TABLE_HEADER_CELL_SORT_DIV_DOWN' ";
						html+=" onclick='SETable.sortForColumn(\"SE_TABLE_ATOMTABLE_DIV_"+sheet_index+"_"+table_index+"\","+(-1-ck)+",\"desc\")'></div>";//↓
			...
			console.log("corner_cols originally "+corner_cols);
		...
		that.format_task[TABLE_DIV_ID]=function(){
			...
			console.log('of_'+sheet_index+"_"+table_index);
			...
			console.log("corner_cols="+corner_cols);
			...
			console.log('@ '+parseInt($("#"+TABLE_DIV_ID+" .SE_TABLE_RIGHT_DIV_BODY").css("width").replace('px','')));
			console.log('@ '+table_config.scroll_size);
			console.log('--> #'+TABLE_DIV_ID+' .SE_TABLE_RIGHT_DIV_BODY.width to be '+($("#"+TABLE_DIV_ID+" .SE_TABLE_RIGHT_DIV_BODY").css("width").replace('px','')/**1+table_config.scroll_size*/)+"px");
	...
	this.getLeftCellCols=function(l){
		var cols=0;
		for(var r in l){
			for(var c in l[r]){
				if(typeof l[r][c] =='object' && l[r][c][colspan]){
					cols+=l[r][c][colspan];
				}else{
					cols+=1;
				}
		...
		console.log(l);
		console.log('left cols = '+cols);
		return cols;
	}

	this.mergeCornerAndHeader=function(c,h){
		// console.log('mergeCornerAndHeader');
		// console.log(c);
		// console.log(h);
		var nh=[];

		if(c instanceof Array){
			for(var r_index in c){
				if(!nh[r_index]){
					nh[r_index]=[];
				}
				nh[r_index].push(c[r_index]);
			}
		}else{
			if(!nh[r_index]){
				nh[r_index]=[];
		...
		for(var r_index in h){
			if(!nh[r_index]){
				nh[r_index]=[];
			}
			nh[r_index].push(h[r_index]);
		}
	...
	this.sortForColumn=function(ID_OF_SE_TABLE_ATOMTABLE_DIV_S_T,base_col_id,sort_method){
		...
		function getLeftTrListItemValue(right_tr_list,row_id){
			var bcid=(-1-base_col_id);
			return (left_tr_list[row_id].children[bcid].children[0].children[0].value);
		}
	    ...
	    if(base_col_id>=0){
			...
			for(var i=0;i<left_tr_list.size();i++){
		    	sortObjs[i]={k:i,v:getLeftTrListItemValue(right_tr_list,i)};
		    }
		}
